#!/usr/bin/env python3
"""
测试 Flow Proxy 图片生成接口。

用法（先启动服务: python -m src.main）:

  # 方式 1：命令行参数
  python scripts/test_generate_image.py ^
    --project-id "你的项目UUID" ^
    --session-token "ya29.xxx" ^
    --prompt "A sunset over mountains" ^
    --aspect-ratio IMAGE_ASPECT_RATIO_LANDSCAPE

  # 方式 2：环境变量（可在项目根 .env 同目录设置）
  set FLOW_PROJECT_ID=...
  set FLOW_SESSION_TOKEN=...
  python scripts/test_generate_image.py

  # 仅健康检查
  python scripts/test_generate_image.py --health-only
"""
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="测试 /api/v1/images/generate")
    parser.add_argument(
        "--base-url",
        default=_base_url(),
    ...
    parser.add_argument(
        "--prompt",
        default=os.getenv("FLOW_PROMPT", "A cinematic landscape at golden hour"),
        help="提示词（或 FLOW_PROMPT）",
    )
    parser.add_argument(
        "--aspect-ratio",
        default=os.getenv(
            "FLOW_IMAGE_ASPECT_RATIO", "IMAGE_ASPECT_RATIO_LANDSCAPE"
        ),
        help="image_aspect_ratio（或 FLOW_IMAGE_ASPECT_RATIO）",
    )
    parser.add_argument(
        "--model-name",
        default=os.getenv("FLOW_IMAGE_MODEL_NAME", "NARWHAL"),
        help="image_model_name（或 FLOW_IMAGE_MODEL_NAME）",
    )
    parser.add_argument(
        "--timeout",
        type=float,
        default=float(os.getenv("FLOW_TEST_TIMEOUT", "300")),
        help="请求超时秒数（打码+生成可能较久，默认 300）",
    )
    ...
    payload = {
        ...
        "prompt": args.prompt,
        "image_aspect_ratio": args.aspect_ratio,
        "image_model_name": args.model_name,
    }
